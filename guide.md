...
This placeholder guide is shipped with `guide-kit`. It exists for two reasons:
...
Code blocks render in a light gray panel in the theme's `--mono-font` — `Guide Mono`, the bundled DejaVu Sans Mono, so the panel looks the same on every host.

::: slide
## What this template gives you

- One Markdown source, three outputs: PDF, website, slides
- Bundled fonts, so a render looks the same on any machine
- A version stamp that says which source built the file you are holding
:::
...
- **`make drift-canary`** — the environmental check. It builds a fresh PDF and compares it to the committed reference on two axes: PDF bytes, and the embedded-font list `pdffonts` reports. Its target is drift that changes the render with no source change at all — a dependency bump, a rebuilt CI image — which the staleness check is blind to by construction, because `pixi.lock` is not a source file. CI runs it on every build and weekly on a schedule. There is no canonical host: the repo bundles its own fonts, so the same commit renders the same bytes anywhere. (`make verify-render` is the older, weaker form — page count plus extracted text — kept as a local convenience.)
...
- Edit `guide.md`
- `make` for the PDF, `make web` for the site, `make slides` for this deck
- `make slides-coverage` says which chapters have no slide yet
:::
