...
web layer INTO the kit and delete the kit's own bootstrap).
    ...
    python guide-template/adopt-web.py --target ../mac-terminal-guide
    python guide-template/adopt-web.py --target ../mac-terminal-guide --with-transforms
...
not refuse the files the kit just wrote (Phase 5 refuses an existing managed dest
...
def _plan(kit_root: Path, target: Path, with_transforms: bool) -> list[_W]:
    ...
    writes: list[_W] = []
    for proj in manifest.projections("web-enabled", slug=target_cfg.OUTPUT_SLUG):
