...
- **`BREAKING.md` names the changes a sync cannot repair on its own**, and `sync.py` prints the ones a guide has not passed yet **before it writes anything**. The gap it closes is a seam in the model rather than a bug in it: sync overwrites kit-owned files and never touches target-owned ones, so it can deliver a stricter `kitconfig.py` while leaving in place the `guide.toml` that version now rejects. Sync succeeds and says so; the next `make` dies on `unknown key`, naming a file sync did not write. Every step is correct and nothing inside sync can see it coming. The range comes from `git rev-list <recorded>..HEAD`, and an unanswerable range (no recorded version, a short SHA, a commit absent from the checkout) lists **every** entry with a caveat rather than going quiet — under-reporting here would reproduce the exact failure the file exists to prevent. `tests/test_breaking_changes.py` pins the accepted `guide.toml` vocabulary, so removing or renaming a setting fails a test whose message tells you to write the entry; a note that depends on remembering to write it is the same failure in a different costume.
