...
deck — built by pandoc + WeasyPrint, rendered identically on any machine from
bundled fonts, and kept honest by a version stamp that says which source built
...
mid-deploy: account id, the Workers Scripts permission, a configured workers.dev
subdomain, and that the slug is a valid DNS label.
...
- **A verify split by question:** `make verify` is the staleness check (embedded stamp vs. a fresh content hash — no build); `make drift-canary` is the environmental check (PDF bytes + the `pdffonts` face list vs. the committed reference); `make smoke` asks whether the PDF looks like a finished guide. CI runs all three.
...
make smoke                      # does the reference PDF look like a finished guide? (no build)
...
| `chapters.py` | Splits the Pandoc AST into chapter units for the `multipage` site and for `make slides-coverage`. A site input, deliberately **not** a PDF one. |
...
| `baseline.py` | Helper for `make baseline` — dirty-tree guard, then promotes the working render onto that artifact's committed reference (`--artifact pdf\|slides`). |
...
| `adopt-web.py` | The older single-purpose form of the same transition — add the web layer to an already-initialized guide (`--target <guide>`). |
...
| `hub.py` | Builds the family hub — the omnibus index site. Only meaningful for `[outputs] site = "hub"`; a guide never runs it. |
...
| `.github/workflows/verify.yml` | CI on Ubuntu, paths-filtered: the kit test suite, the PDF and web build smokes, `make smoke`, `make verify` (the staleness check), the drift canary, and the `baseline.yml` dispatch when a push leaves the reference stale. Never runs `make verify-render`. |
...
For **doc-only changes** — anything outside the authorable set — the rendered PDF is unaffected. Commit normally; no reference refresh needed. This covers `README.md`, `CLAUDE.md`, `LICENSE*`, `Makefile`, `pixi.toml`, `pixi.lock`, `verify_artifacts.py`, `driftcanary.py`, `baseline.py`, `release.py`, `bootstrap.py`, `guidekit.py`, `sync.py`, `adopt.py`, `adopt-web.py`, `hub.py`, `pdfaudit.py`, `kit-manifest.toml`, `kitmanifest.py`, `.template-version`, and `.github/workflows/`. `release.py` enforces this boundary — it refuses to run when modifications outside the authorable set are present, so a doc edit can never accidentally hitchhike into a release commit.
...
whole vocabulary rather than a selection from it. `tests/test_documented_config_loads.py`
feeds this block to the real loader, so it cannot drift from the schema.
...
AUTHOR = "A. Author"
DESCRIPTION = "One sentence; bootstrap appends the CC-BY boilerplate."
...
The site build copies the two it depends on into the built tree — flattened into one `assets/`
directory, so a single `![](assets/x.png)` in `guide.md` resolves in the PDF (against the repo
root) *and* on the site (against the served tree). `assets/print/**` is deliberately not
published: shipping bytes the site's own closure hash does not cover is how a deploy stops
describing itself.

Two things here are easy to miss: **`make smoke`** asks whether the reference PDF looks like a
...
A fourth command answers a question none of the three do: **`make smoke`** asks whether the PDF *looks like a finished guide* — the check that would have caught a footer wrapping onto a second line on every page. It is build-free and platform-independent, so CI runs it too.

## CI policy

CI (GitHub Actions) runs on `ubuntu-latest` only: the **kit test suite**, a **PDF build smoke** (`make` renders without crashing), a **web build smoke** (`make web`, when the guide declares a site), **`make smoke`** (does the reference PDF look like a finished guide?), and **`make verify`** (the staleness check). Because staleness is computed from hashes rather than rendered, it reproduces identically on any machine, so — unlike the old image-comparison harness — it is a real gate in CI, not just a local one. CI runs `driftcanary.py` rather than `make verify-render`, reusing the PDF the build smoke already produced: it is the stronger of the two, comparing bytes and the embedded-face list rather than page count and text.
...
That materializes `style-screen.css`, activates `transforms.py` (the per-output YouTube embed split, so embeds work on the site and degrade to links in the PDF), copies the `app/` Cloudflare scaffold (with your slug as the worker name), and activates a live `.github/workflows/deploy.yml`. (Already initialized without it? Declare `[outputs] site` and its `[artifacts.site]` date in the guide's `guide.toml`, commit that, then run `python guide-kit/adopt.py --target ../my-guide --output site --enable` — it materializes the same web layer transactionally and records the new managed files in `.template-version` so `sync.py` doesn't later refuse them. `adopt-web.py` is the older single-purpose form of the same thing. `transforms.py` is **not** activated by default, since it is a SOURCE_FILE and would shift the version stamp; pass `--with-transforms` if the guide needs it, then re-baseline with `make release`.)

The built tree is **provider-neutral** and that is proven by serving it: `tests/test_static_portability.py` starts a plain `http.server` over the exact `app/dist/` output and exercises the routes, the PDF's media type and 404 semantics. The one Cloudflare-specific artifact is `_headers`, which `cfadapter.py` writes — so forced download is provider-*optional*. On a host that ignores `_headers` the PDF opens inline instead of downloading, and nothing else differs.
