...
def test_fresh_passes(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(verify_artifacts, "read_stamp", lambda pdf: (fresh, False))
    assert verify_artifacts.staleness_check(repo) == 0
...
def test_dirty_stamped_reference_fails(tmp_path, monkeypatch, capsys):
    ...
    repo = _mkrepo(tmp_path)
    fresh = kitconfig.content_hash(repo)
    monkeypatch.setattr(verify_artifacts, "read_stamp", lambda pdf: (fresh, True))
    assert verify_artifacts.staleness_check(repo) == 1
...
def test_edited_source_fails_and_names_file(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setattr(verify_artifacts, "read_stamp", lambda pdf: (baseline_hash, False))
...
def test_no_readable_stamp_fails(tmp_path, monkeypatch, capsys):
    repo = _mkrepo(tmp_path)
    monkeypatch.setattr(verify_artifacts, "read_stamp", lambda pdf: (None, False))
    assert verify_artifacts.staleness_check(repo) == 1
...
def test_parse_stamp_hash_requires_date_prefix():
    ...
    assert verify_artifacts.parse_stamp_hash("2026-01-02 · abcdef123456") == "abcdef123456"
    assert verify_artifacts.parse_stamp_hash("2026-01-02 · abcdef123456 · dirty") == "abcdef123456"
    assert verify_artifacts.parse_stamp_hash("no stamp here") is None
    ...
    assert verify_artifacts.parse_stamp_hash("see example · deadbeefcafe here") is None
    assert verify_artifacts.parse_stamp_hash(
        "example · deadbeefcafe\n2026-01-02 · abcdef123456"
    ) == "abcdef123456"
