...
def _guide_toml(slug: str, site: str = "single") -> str:
    return (
        ...
        '[outputs]\n'
        'pdf = true\n'
        # Config-first: the site is DECLARED before adopt-web materializes it,
        # or sync would not manage the files it writes.
        f'site = "{site}"\n'
        'slides = false\n'
        '[artifacts.pdf]\n'
        'date = "2026-07-26"\n'
...
def _min_target(dst: Path, slug: str = "min-guide") -> Path:
    ...
    (dst / ".template-version").write_text(json.dumps({
        "schema_version": 1, "source_repo": "rosslevinsky/guide-kit", "kit_version": "t",
...
def test_records_checksums_so_sync_does_not_refuse(tmp_path):
    ...
    r = subprocess.run(
        [sys.executable, str(fork / "bootstrap.py"), "Mac Guide", "mac-guide",
         "--source-repo", "rosslevinsky/guide-kit", "--kit-version", "t"],
        cwd=fork, capture_output=True, text=True,
    )
    assert r.returncode == 0, r.stderr

    # Config-first: the fork DECLARES the site and commits that, then
    # adopts. adopt-web never writes guide.toml — it is target-owned
    # (`policy = "never"`), so the declaration is the user's edit, not the
    # tool's side effect.
    toml = fork / "guide.toml"
    toml.write_text(
...
def test_refuses_when_the_site_is_not_declared(tmp_path):
    """Config-first. sync resolves managed destinations from the DECLARED
    shape, so materializing the web layer while `outputs.site = "none"` would
    write files sync then refuses to see: upstream web changes would never reach
    the target and drift in them would never be reported."""
    kit = _mkkit(tmp_path / "kit")
    target = _min_target(tmp_path / "tgt")
    (target / "guide.toml").write_text(_guide_toml("min-guide", site="none"), encoding="utf-8")

    with pytest.raises(adopt_web.AdoptError, match="outputs.site"):
        adopt_web.adopt_web(kit, target)
