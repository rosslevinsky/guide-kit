"""Anchor the verify / verify-render split.
...
def test_strip_stamp_removes_dated_footer_but_preserves_body_lookalike():
    ...
    stripped = verify_artifacts.strip_stamp(text)
...
def test_verify_artifacts_imports_no_renderer():
    ...
    src = (REPO_ROOT / "verify_artifacts.py").read_text(encoding="utf-8")
...
def test_ci_runs_the_staleness_check_never_verify_render():
    ...
    `verify_artifacts.py --staleness` call. The direct form is not a style choice: GNU
    ...
    and `verify-render` never runs in CI — now because `driftcanary.py` asks the
    same question more strongly off the render already built, not because
    `verify-render` is platform-sensitive (bundled fonts ended that).
    """
    ...
    staleness = [
        ...
        or re.search(r"verify_artifacts\.py\s+--staleness\b", r)
    ]
    assert staleness, "CI must run the staleness check (`make verify` or verify_artifacts.py --staleness)"
